...
| TC-CFG-27 | Add by a link where the request to the source site fails with a real network error (DNS doesn't resolve, SSRF block, etc.) | The log shows `[providers] <context> error url="…" message="…"` with the actual error text (not silence); also a `[providers] no usable metadata …` line showing what the specific provider and generic parsing each returned |
| TC-CFG-28 | Add by a link that just isn't found (a valid 404, not a network error) | No `[providers] … error` line (network exception) in the log, but there is `[providers] og-scrape non-ok response url="…" status=404 …` — the fact and status code are visible, it's just not treated as a network failure |
| TC-CFG-29 | Add by a link to a page that's detected as anti-bot/captcha (see TC-CLN, section 4) | The log shows `[providers] og-scrape looks blocked url="…" title="…"` with the exact title that triggered the detection |
