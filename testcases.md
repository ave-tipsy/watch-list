...
| TC-CFG-28 | Add by a link that just isn't found (a valid 404, not a network error) | No `[providers] … error` line in the log — only the summary `no usable metadata` line; an HTTP error response isn't treated as a network failure |
